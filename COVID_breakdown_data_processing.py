...
class MainSheet(Template):
  ...
  def getOnsetDate(self):
    ...
    for i, onset_date in enumerate(self.getCol(self.coltag_onset_date)):
      if onset_date != onset_date: ## NaN
        ...
        '5/14-22', '5/14-29', '5/15-26', '5/15-6/4', '5/20-30', '5/20-31', '5/18-6/2', 
  ...
  def getChannel(self):
    ...
    for i, channel in enumerate(self.getCol(self.coltag_channel)):
      ...
      elif channel in ['居家隔離', '住院隔離', '接觸患者', '同院患者', '框列採檢', '接觸者回溯', '接觸者檢查', '接觸者採檢', '確診者回溯']:
  ...
  def getSymptom(self):
    key_dict = {
      ...
      'symptomatic': ['全身不舒服', '出現症狀', '身體不適', '有症狀', '不舒服', '不適'] + \
        ['血氧濃度54%', '血氧降低', '排尿疼痛', '眼球上吊', '肢體變黑', '鼻子乾', '猝死', '手抖', '吐血'],
  ...
  def getLink(self):
    ...
    for i, link in enumerate(self.getCol(self.coltag_link)):
      if link == '未知':
        ...
        '遠傳案', '朝陽夜唱', '金沙酒店', '金樽喜宴', '泰安附幼', '成功市場', '洗腎診所', '銀河百家樂', '維納斯會館', '羅東遊藝場', '串門子餐廳', 
