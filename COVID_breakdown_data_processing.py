...
class MainSheet(Template):
  ...
  def getOnsetDate(self):
    ...
    for i, onset_date in enumerate(self.getCol(self.coltag_onset_date)):
      if onset_date != onset_date: ## NaN
        ...
        '1月', '2/18-25', '3月', '4/6-5/15', '4/25-5/22', '4/26-5/26', '4/29-5/27', '4/29-5/30', '4/30-5/18', '5/1-19', '5/2-13', '5/5-16', '5/5-17', '5/6-22', '5/6-27', 
        '5/7-20', '5/7-28', '5/8-20', '5/10-16', '5/10-5/18', '5/10-20', '5/10-21', '5/10-23', '5/14-22', '5/2-22', '5/7-24', '5/7-25', '5/8-25', '5/11-27', 
        '5/13-25', '5/13-27', '5/14-29', '5/15-26', 
  ...
  def getChannel(self):
    ...
    for i, channel in enumerate(self.getCol(self.coltag_channel)):
      ...
      elif channel in ['居家隔離', '接觸者回溯', '接觸者檢查', '接觸者採檢', '確診者回溯']:
        channel_list.append('isolation')
        
      elif channel in ['自主健康管理', '加強自主管理']:
        channel_list.append('monitoring')
        
      elif channel in ['自行就醫',  '自主就醫', '自費篩檢', '自費採檢', '自費檢驗', '自行通報', '接觸患者', '同院患者', '入院篩檢', '社區快篩', '社區專案', '萬華專案', '入院']:
  ...
  def getSymptom(self):
    key_dict = {
      ...
      'fatigue': ['全身倦怠無力', '全身倦怠', '全身疲憊', '身體無力', '全身無力', '四肢無力', '體力不支 左側肢體無力', '疲倦感', '倦怠情', '倦怠', '疲憊', '疲倦', '疲勞', '無力', '虛弱'],
      ...
      'dysnosmia': ['嗅覺異常症狀', '失去嗅覺', '嗅覺不靈敏', '嗅覺喪失', '嗅覺變差', '嗅覺遲鈍', '嗅覺異常', '喪失嗅覺', '無嗅覺'], 
      ...
      'symptomatic': ['全身不舒服', '出現症狀', '身體不適', '有症狀', '不舒服', '不適'] + ['排尿疼痛', '眼球上吊', '鼻子乾', '猝死', '吐血'],
  ...
  def getLink(self):
    ...
    for i, link in enumerate(self.getCol(self.coltag_link)):
      if link == '未知':
        ...
        '遠傳案', '朝陽夜唱', '金沙酒店', '金樽喜宴', '泰安附幼', '成功市場', '銀河百家樂', '維納斯會館', '羅東遊藝場', '串門子餐廳', '中國醫K歌團', '小姑娘小吃店', '快樂城小吃店', 
        '接觸確診個案', '接觸其他確診者', '東方紅時尚會館', 
      ]:
        link_list.append('linked')
