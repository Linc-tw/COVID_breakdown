...
function ASC_ResetText() {
  if (LS_lang == 'zh-tw') {
    LS_AddStr('age_symptom_correlations_title', '個案年齡與症狀相關程度');
    LS_AddStr('age_symptom_correlations_button_1', '相關係數');
    LS_AddStr('age_symptom_correlations_button_2', '案例數');
  }
  
  else if (LS_lang == 'fr') {
    LS_AddStr('age_symptom_correlations_title', 'Corrélations entre âge & symptômes');
    LS_AddStr('age_symptom_correlations_button_1', 'Coefficients');
    LS_AddStr('age_symptom_correlations_button_2', 'Nombres');
  }
  
  else { //-- En
    LS_AddStr('age_symptom_correlations_title', 'Correlations between Age & Symptoms');
    LS_AddStr('age_symptom_correlations_button_1', 'Coefficients');
    LS_AddStr('age_symptom_correlations_button_2', 'Counts');
  }
}
...
function ASC_FormatData2(wrap, data2) {
  var i, row, n_total, n_data;
  
  //-- Get value of `n_total` & `n_data`
  //-- Loop over row
  ...
  for (i=2+wrap.y_list.length; i<2+wrap.y_list.length+wrap.x_list.length; i++) {
    row = data2[i];
    count = row['count'];
  ...
  for (i=2; i<2+wrap.y_list.length; i++) {
    row = data2[i];
    count = row['count'];
...
function ASC_Plot(wrap) {
  ...
  wrap.svg.selectAll()
    ...
    .append('rect')
      ...
      .attr('y', function (d) {return wrap.yscale(d.age);})
      .attr('rx', 3)
      .attr('ry', 3)
  ...
  wrap.svg.selectAll()
    ...
    .append('text')
      ...
      .attr('y', function (d) {return wrap.yscale(d.age) + 0.5*+wrap.yscale.bandwidth();})
