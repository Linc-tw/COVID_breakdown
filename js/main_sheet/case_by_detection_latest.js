...
//-- Variables
CBD_latest_wrap.do_cumul = document.querySelector("input[name='" + CBD_latest_wrap.tag + "_cumul']:checked").value;
CBD_latest_wrap.do_onset = document.querySelector("input[name='" + CBD_latest_wrap.tag + "_onset']:checked").value;
...
//-- Buttons
GS_PressRadioButton(CBD_latest_wrap, 'cumul', 0, CBD_latest_wrap.do_cumul); //-- 0 from .html
GS_PressRadioButton(CBD_latest_wrap, 'onset', 0, CBD_latest_wrap.do_onset); //-- 0 from .html

$(document).on("change", "input:radio[name='" + CBD_latest_wrap.tag + "_cumul']", function (event) {
  GS_PressRadioButton(CBD_latest_wrap, 'cumul', CBD_latest_wrap.do_cumul, this.value);
  CBD_latest_wrap.do_cumul = this.value;
  CBD_Latest_Replot();
});

$(document).on("change", "input:radio[name='" + CBD_latest_wrap.tag + "_onset']", function (event) {
  GS_PressRadioButton(CBD_latest_wrap, 'onset', CBD_latest_wrap.do_onset, this.value);
...
d3.select(CBD_latest_wrap.id + '_save').on('click', function (){
  ...
  if (CBD_latest_wrap.do_cumul == 1)
    tag1 = 'cumulative';
  else
    tag1 = 'daily';
  
  if (CBD_latest_wrap.do_onset == 1)
    tag2 = 'onset';
  else
    tag2 = 'report';
