...
//-- Variables
DBT_latest_wrap.col_ind = document.querySelector("input[name='" + DBT_latest_wrap.tag + "_ind']:checked").value;
...
//-- Buttons
GS_PressRadioButton(DBT_latest_wrap, 'ind', 0, DBT_latest_wrap.col_ind); //-- 0 from .html

$(document).on("change", "input:radio[name='" + DBT_latest_wrap.tag + "_ind']", function (event) {
  GS_PressRadioButton(DBT_latest_wrap, 'ind', DBT_latest_wrap.col_ind, this.value);
...
d3.select(DBT_latest_wrap.id + '_save').on('click', function () {
  ...
  if (DBT_latest_wrap.col_ind == 0)
    tag1 = 'all';
  else if (DBT_latest_wrap.col_ind == 1)
    tag1 = 'imported';
  else if (DBT_latest_wrap.col_ind == 2)
    tag1 = 'local';
  else
    tag1 = 'others';
