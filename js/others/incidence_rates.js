    ...
    dot.selectAll('.content.dot')
      ...
      .attr('class', 'content dot')
        ...
        .attr('r', 0)
          ...
          .on('mousemove', function (d) {IR_MouseMove(wrap, d);})
...
function IR_Replot(wrap) {
  ...
  var legend_label;
  if (LS_lang == 'zh-tw')
    legend_label = ['入境盛行率', '本土盛行率（乘以1000）'];
  else if (LS_lang == 'fr')
    legend_label = ["Taux d'incidence frontalier", "Taux d'incidence local (multiplié par 1000)"];
  else
    legend_label = ['Arrival incidence', 'Local incidence (multiplied by 1000)'];
