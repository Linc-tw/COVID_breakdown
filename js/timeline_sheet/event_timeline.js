...
function ET_InitFig(wrap) {
  ...
  var even_year = +wrap.id.slice(-4);
  if (even_year == 2020) {
    wrap.tot_height_['zh-tw'] = 750;
    wrap.tot_height_['fr'] = 750;
    wrap.tot_height_['en'] = 750;
  }
  else if (even_year == 2022) {
    wrap.tot_height_['zh-tw'] = 750;
    wrap.tot_height_['fr'] = 750;
    wrap.tot_height_['en'] = 750;
  }
...
function ET_ResetText() {
  if (LS_lang == 'zh-tw') {
    LS_AddStr('event_timeline_2020_title', '2019-2020 疫情時間軸');
    LS_AddStr('event_timeline_2020_button_sun', '週日為首');
    LS_AddStr('event_timeline_2020_button_mon', '週一為首');
    ...
    LS_AddStr('event_timeline_2020_title', 'Chronologie de la pandémie 2019-2020');
    LS_AddStr('event_timeline_2020_button_sun', '1er jour dimanche');
    LS_AddStr('event_timeline_2020_button_mon', '1er jour lundi');
    ...
    LS_AddStr('event_timeline_2020_title', 'Pandemic Timeline 2019-2020');
    LS_AddStr('event_timeline_2020_button_sun', '1st day Sunday');
    LS_AddStr('event_timeline_2020_button_mon', '1st day Monday');
...
function ET_FormatData(wrap, data) {
  ...
  var lim = 44; //-- Number of characters
  var formatted_data = {}
  var row, text_1, text_2, text_3, split_a1, split_a2, split_a3, split_b1, split_b2, split_b3;
  var i, j, k, q, r, str_;
  ...
  var even_year = +wrap.id.slice(-4);
  if (even_year > 2020) {
    begin_year = even_year / 2 * 2 - 1;
    begin_month = 1;
  }
  if (even_year / 2 < end_year / 2) {
    end_year = even_year;
    end_month = 12;
  }
...
function ET_SpecialProcess(wrap, formatted_data) {
  formatted_data['2019-12-30'][2] = [
    ...
    '★ 檢驗所把檢驗報告(12/27)發回醫院，報告中將「SARS coronavirus」（SARS冠狀病毒）列為',
    '「高置信度」陽性指標的結果之一',
  ...
  formatted_data['2019-12-31'][1] = [
    ...
    'News resource today indicate that at least seven atypical pneumonia cases were reported in',
    'Wuhan, CHINA.',
    'Their health authorities replied to the media that the cases were believed not SARS; however the',
    'sample are still under examination, and cases have been isolated for treatment.',
  ...
  formatted_data['2019-12-31'][3] = [
    '',
    'CDC時間軸', 
    '★ 疾病管制署自網路上得知，在中國武漢市發生至少七例非典型肺炎(atypical pneumonia)。',
    '疾病管制署當日已向中國疾控中心及世界衛生組織(WHO)IHR窗口確認疫情訊息。',
    '★ 陸方於傍晚回應已派專家赴武漢，進行病原檢測及感染原因調查，並提供武漢市衛生健康委員會',
    '12月31日之公布訊息，指出近期當地發現27例肺炎病例，7例病情危重，均已進行隔離治療，有2例',
    '病情好轉，臨床表現主要為發燒，少數病人呼吸困難，呈雙肺浸潤性病灶，大部分病例為武漢市江漢',
    '區華南海鮮城經營戶，尚未發現明顯人傳人現象及醫護人員感染。',
    '★ 我國通報世界衛生組織(WHO)電郵內容事實陳述之聲明',
    '★ WHO駐中國國家代表處從武漢市衛生委員會的網站上獲取了關於中國武漢市“病毒性肺炎”病例的',
    '媒體聲明。',
    '★ 因應中國武漢市之肺炎流行疫情，為防範病例自境外移入，我國已依標準作業程序啟動邊境檢疫應',
    '變措施，包括：',
    '1.加強入境旅客之發燒篩檢。 ',
    '2.落實疑似病例之旅遊史、職業別、接觸史、群聚情形等詢問、健康評估及衛教宣導。 ',
    '3.評估疫情可有效阻絕於境外。 ',
    '4.自12月31日起將針對自中國武漢直航入境之班機進行登機檢疫，除主動評估旅客健康情形外，亦於',
    '班機上向旅客宣導相關預防措施。'
  ]
