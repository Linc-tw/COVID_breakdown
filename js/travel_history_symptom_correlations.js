...
function THSC_Initialize(wrap) {
  ...
  wrap.svg.selectAll()
    ...
    .append("text")
      ...
      .style("fill", function (d) {if (Math.abs(d['value'])<0.3) return '#000'; return '#fff';})
...
function THSC_Update(wrap) {
  ...
  //-- Legend - label
  var legend_label;
  if (GS_lang == 'zh-tw') legend_label = ['有資料案例數', '資料不全', '無旅遊史', '合計'];
  else if (GS_lang == 'fr') legend_label = ['Données complètes', 'Données incomplètes', 'Sans anté. de voyage', 'Total'];
