...
class CaseSheet(ccm.Template):
  ...
  def getChannel(self):
    ...
    for i, channel in enumerate(self.getCol(self.coltag_channel)):
      if channel != channel: ## Is nan
        ...
        '社區快篩', '社區專案', '社區篩檢', '專案篩檢', '登船檢疫',
        '萬華專案', '擴大採檢', '擴大篩檢', '出國前採檢', '預防性快篩', '預防性採檢', 
  ...
  def getSymptom(self):
    key_dict = {
      ...
      'cough': ['輕微咳嗽', '咳嗽症狀', '咳嗽加劇', '咳嗽併痰', '咳嗽有痰', '痰有血絲', '喉嚨有痰', '有點咳嗽', '偶爾咳嗽', '咳嗽', '乾咳', '輕咳', '有痰', '痰'],
      'throatache': [
      ...
      'fever': [
        '出現中暑的狀態', '身體悶熱不適', '間歇性發燒', '身體微熱', '體溫偏高', '體溫升高', '反覆發燒', '身體發熱', '臉紅微熱',
        '微燒', '低燒', '高燒', '發燒', '發熱', '盜汗'
      ], 
      ...
      'soreness': [
        ...
        '全身痠痛', '全身酸痛', '骨頭痠痛', '骨頭酸痛', '關節痠痛', '身體痠痛', '四肢痠痛', '手部痠痛', '肌肉痛', '骨頭酸', '關節痛', '身體痛', '痠痛'
      ],
      'hypersomnia': ['嗜睡'],
      'insomnia': ['不易入睡', '睡不著'], 
      ...
      'symptomatic': ['全身不舒服', '出現症狀', '身體不適', '樓紅不適', '有症狀', '不舒服', '活動差', '不適'] + \
  ...
  def getLink(self):
    ...
    for i, link in enumerate(self.getCol(self.coltag_link)):
      if link == '未知':
        ...
        '國軍桃園總醫院', '桃園國軍總醫院', '台北家禽批發場', '北部幼兒園羣聚', '北部幼兒園及社區羣聚',
